<template>
  <div class="mx-3 md:mx-auto">
    <div class="mb-4 max-w-lg text-center mx-auto">
      <div>
        <label for="filter" class="text-gray-500"
        ...
        <input
          ...
          class="my-2 w-full rounded-md shadow-sm border-gray-300 focus:border-violet-300 focus:ring focus:ring-violet-200 focus:ring-opacity-50"
          autofocus
        />
      </div>
      <div>
        <select
          v-model="selection"
          class="my-2 w-full rounded-md shadow-sm border-gray-300 focus:border-violet-300 focus:ring focus:ring-violet-200 focus:ring-opacity-50 text-sm"
        >
      ...
      <div>
        <span
          v-if="count === 0"
          class="mx-1 md:mx-2 px-2 py-0.5 text-xs rounded-full font-semibold text-slate-600 bg-slate-400/10"
          >Update times in ms ...</span
        >
        <span
          v-if="count === 0"
          class="mx-1 md:mx-2 px-2 py-0.5 text-xs rounded-full font-semibold text-green-600 bg-green-400/10"
          >&le; 10</span
        >
        <span
          v-if="count === 0"
          class="mx-1 md:mx-2 px-2 py-0.5 text-xs rounded-full font-semibold text-amber-600 bg-amber-400/10"
          >&le; 30</span
        >
        <span
          v-if="count === 0"
          class="mx-1 md:mx-2 px-2 py-0.5 text-xs rounded-full font-semibold text-red-600 bg-red-400/10"
          >&gt; 30</span
        >
        <span
          v-for="i in count"
